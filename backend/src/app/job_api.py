...
async def start_production(
    ...
    graph = PipelineGraphService()
    job_ids = await graph.create_graph(session, film_id, environment.environment_id, request.payload)
...
async def list_jobs(
    ...
    return [
        {"job_id": j.job_id, "job_type": j.job_type, "status": j.status, "attempts": j.attempts, "max_attempts": j.max_attempts, "error_code": j.error_code}
        for j in jobs
    ]
