...
async def start_production(film_id: UUID, request: StartProductionRequest, principal: Principal = Depends(get_principal), session: AsyncSession = Depends(get_db)):
...
async def enqueue_job(film_id: UUID, request: EnqueueJobRequest, principal: Principal = Depends(get_principal), session: AsyncSession = Depends(get_db)):
    _, environment = await _film_context(session, film_id, principal)
    ...
    job = await PostgresJobRepository(session).create(film_id, environment.environment_id, request.job_type, request.payload)
    await session.commit()
...
async def list_jobs(film_id: UUID, principal: Principal = Depends(get_principal), session: AsyncSession = Depends(get_db)):
    ...
    return [{"job_id": j.job_id, "job_type": j.job_type, "status": j.status, "attempts": j.attempts, "max_attempts": j.max_attempts, "error_code": j.error_code, "result": j.result} for j in jobs]
...
async def get_job(job_id: UUID, principal: Principal = Depends(get_principal), session: AsyncSession = Depends(get_db)):
    ...
    return {"job_id": job.job_id, "film_id": job.film_id, "environment_id": job.environment_id, "job_type": job.job_type, "status": job.status, "attempts": job.attempts, "max_attempts": job.max_attempts, "error_code": job.error_code, "result": job.result, "created_at": job.created_at, "updated_at": job.updated_at, "started_at": job.started_at, "completed_at": job.completed_at}
